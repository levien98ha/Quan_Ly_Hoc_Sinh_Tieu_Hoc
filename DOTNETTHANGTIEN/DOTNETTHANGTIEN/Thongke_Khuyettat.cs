...
{
    public partial class Thongke_Khuyettat : Form
    {
        DataClasses3DataContext db = new DataClasses3DataContext();
        public Thongke_Khuyettat()
        {
            InitializeComponent();
        }
        void loadKhoi()
        ...
        private void Thongke_Khuyettat_Load(object sender, EventArgs e)
        {
            loadKhoi();
            LoadSchool();
        }
        ...
        private void cbKhoi_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var y = from hs in db.BANGHOCSINHs
                        where (hs.Khuyết_tật == true && hs.BANGLOPHOC.BANGKHOI.Tên_khối == cbKhoi.SelectedItem.ToString())
                        select new
                        {
                            hs.Tên_học_sinh,
                            hs.BANGLOPHOC.Tên_lớp,
        ...
        private void cbLop_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var y = from hs in db.BANGHOCSINHs
                        where (hs.Khuyết_tật == true && hs.BANGLOPHOC.Tên_lớp == cbLop.SelectedItem.ToString())
                        select new
                        {
                            hs.Tên_học_sinh,
                            hs.BANGLOPHOC.Tên_lớp,
